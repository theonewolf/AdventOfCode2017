...
# vim:set nospell:
"""
Implements the fourth challenge, part two.
...
def isvalid(passphrase: str) -> bool:
    """
    Solves the AOC fourth puzzle, part two.
    """

    passphrase = [''.join(sorted(s)) for s in passphrase.split()]
...
def test_isvalid() -> None:
    """
    This tests the isvalid function by running through a list of
    hard-coded test cases.
    """
    for test in TESTS:
        print(test)
        assert isvalid(test[0]) == test[1]


if __name__ == '__main__':
    test_isvalid()

    TARGET = '''una bokpr ftz ryw nau yknf fguaczl anu
...
lpyubo dylpfb iehwug decj ntidy cuygyg lalkb iutu oxgm imn'''
    ...
    print(sum([isvalid(t) for t in TARGET.split('\n')]))
