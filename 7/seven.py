...
# vim:set nospell:
"""
Implements the seventh challenge.
...
def root(tree: str) -> Set[str]:
    """
    Solves the AOC seventh puzzle.
    """
    ...
    for line in tree.splitlines():
        ...
        if len(line) == 2:
            ...
            for node in line[1].split(','):
                notroot.add(node.strip())

    return mayberoot - notroot


def test_root() -> None:
    """
    This tests the root function by running through a list of
    hard-coded test cases.
    """
    for test in TESTS:
        assert root(test[0]) == test[1]
